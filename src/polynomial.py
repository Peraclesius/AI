...
def generate_coefficients():
    ...
    return [random.uniform(-0.5, 0.5) for _ in range(5)]
...
def generate_data(m, coefficients):
    ...
    """
    # TODO You have to write this
    X = np.linspace(-5.0, 5.0, m).reshape(-1, 1)  #reshaping to column vector
    a, b, c, d, e = coefficients
    y = a + b * X + c * X ** 2 + d * X ** 3 + e * X ** 4 + np.random.normal(0, 1, size=(m, 1))
    return X, y
